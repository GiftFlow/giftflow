...
DELIMITER $$
CREATE  OR REPLACE VIEW `events_view` AS select `E`.`id` AS `id`,`ET`.`title` AS `event_type`,`E`.`data` AS `data`,`E`.`user_id` AS `user_id`,`E`.`transaction_id` AS `transaction_id`,`E`.`message_id` AS `message_id`,`E`.`created` AS `created` from (`testing`.`events` `E` join `testing`.`event_types` `ET` on((`E`.`event_type_id` = `ET`.`id`))) order by `E`.`id`
$$
DELIMITER ;
...
DELIMITER $$
CREATE  OR REPLACE VIEW `notifications_view` AS select `N`.`id` AS `id`,`N`.`event_id` AS `event_id`,`ET`.`title` AS `event_type`,`N`.`user_id` AS `user_id`,`N`.`enabled` AS `enabled`,`N`.`created` AS `created` from ((`testing`.`notifications` `N` join `testing`.`events` `E` on((`N`.`event_id` = `E`.`id`))) join `testing`.`event_types` `ET` on((`E`.`event_type_id` = `ET`.`id`))) order by `N`.`id`
$$
DELIMITER ;
